...
- `llm-canary validate suite.yaml` — lint the suite (typos in provider or
  assertion names, missing judge) without calling anything / 何も呼ばずに
  スイートの誤りを検出
- `--max-workers 8` — run cases concurrently against slow real APIs / 並列実行
- `--json` — machine-readable results / 機械可読な結果出力
- Direct-API mode can load your app's real system prompt so it is on the
  tested path / 直接APIモードでもアプリの実システムプロンプトを経路に乗せる:
